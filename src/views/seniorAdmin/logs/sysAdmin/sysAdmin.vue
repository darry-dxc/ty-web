<template>
  <div>
    <a-table
      :rowKey="record => record.administratorLogId"
      :columns="columns"
      :dataSource="data"
      :loading="loading"
      :pagination="false"
...
export default {
  ...
  methods: {
    ...
    fetch (params = {}) {
      ...
      this.$api.logs.sysAdminLogs(params).then(
        res => {
          console.log(res)
          if (res.code === 200) {
    ...
    exportExcel() {
      ...
      delete submitValues.operateTime
      this.$api.logs.sysAdminLogsExport(submitValues).then(res => {
        if(res.code === 200) {
          const fields = this.$XLSX.utils.json_to_sheet(res.data)
          const wb = this.$XLSX.utils.book_new()
          this.$XLSX.utils.book_append_sheet(wb, fields, 'sheet1')
          let name = '系统管理员日志' + this.$moment(new Date()).format('YYYY-MM-DD') +'.xlsx'
