...
<template>
  <div>
    ...
    />
    <action ref="action" @newDriverSuccess="handleSubmit" @updateDriverSuccess="handleSubmit" @completeDriverSuccess="completeQualificate"/>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    if(process.env.NODE_ENV === 'production') {
      this.bulkUploadAction = baseUrl + 'entrystaff/qualification/import/driver'
    } else {
      this.bulkUploadAction = '/api/entrystaff/qualification/import/driver'
    }
    this.handleSubmit()
  ...
  methods: {
    ...
    completeQualificate () {
      ...
      this.form.validateFields((err, values) => {
        ...
        this.loading = true
        this.$api.qualification.getDriverByCriteria2(values).then((res) => {
          if (res.code === 200) {
            const pagination = { ...this.pagination }
            pagination.total = res.data.total
            this.pagination = pagination
    ...
    downloadExcel() {
      const data = [{
        ...
        '手机号码1': '18888888888',
        '手机号码2': '(可不填)',
        '驾驶证编号': '511502199404230868',
        '驾驶证有效期止': '2019/1/1',
        '道路危险货物运输驾驶证编号': '123456789',
        '道路危险货物运输驾驶证有效期止': '2019/1/1'
      }]
