  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (this.isEdit) {
          ...
          if (this.isComplete){
            ...
            console.log(res)
            if (res.code == 200) {
              this.$message.success('成功完善车头基本信息!')
              this.$emit('completeHeadstockSuccess')
              this.visible = false
            } else {
    ...
    beforeDrivingLicenceAttachmentUpload(file) {
      return new Promise((resolve, reject) => {
        if(this.drivingLicenceFileList.length !== 0 && this.drivingLicenceFileList[0].name !== 'null') {
    ...
    beforeRoadTransLicenceAttachmentUpload(file) {
      return new Promise((resolve, reject) => {
        if(this.roadTransLicenceFileList.length !== 0 && this.roadTransLicenceFileList[0].name !== 'null') {
