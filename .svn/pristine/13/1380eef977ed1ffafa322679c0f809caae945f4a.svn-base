  ...
  methods: {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (this.isEdit) {
          if (this.isComplete){
            ...
            if (res.code == 200) {
              this.$message.success('成功完善挂车基本信息!')
              this.$emit('completeTrailerSuccess')
              this.visible = false
            } else {
    ...
    beforeDangerChemLicenceAttachmentUpload(file) {
      return new Promise((resolve, reject) => {
        if(this.dangerChemLicenceFileList.length !== 0 && this.dangerChemLicenceFileList[0].name !== 'null') {
    ...
    beforeTankInspectionAttachmentUpload(file) {
      return new Promise((resolve, reject) => {
        if(this.tankInspectionFileList.length !== 0 && this.tankInspectionFileList[0].name !== 'null') {
